...
export function validatePemLabel(label) {
  if (typeof label !== 'string' || !LABEL.test(label)) {
    throw new CodecError('pemLabel', 'Use a nonempty PEM label with printable ASCII characters and single spaces or hyphens.');
  }
  return label;
}
...
export function unwrapPem(input) {
  ...
  if (!begin || !end || lines.length < 3) {
    throw new CodecError('pemBoundaries', 'Invalid PEM boundaries. Paste one complete BEGIN/END block or only its Base64 payload.');
  }
  validatePemLabel(begin[1]);
  if (begin[1] !== end[1]) throw new CodecError('pemMismatch', 'PEM BEGIN and END labels must match.');
  const body = lines.slice(1, -1).join('\n');
  if (body.includes('-----')) throw new CodecError('pemMultiple', 'Decode one PEM block at a time.');
  if (/^[^\r\n]*:/m.test(body)) throw new CodecError('pemLegacy', 'Legacy PEM headers are not supported. Paste only the Base64 payload.');
  return body;
}
